...
df = yf.download('KEN.TA', start, end)
df.to_csv("data/'KEN.TA.csv")
df = pd.read_csv("data/'KEN.TA.csv", header=0, index_col=0, parse_dates=[0])
print(df.head())
...
KEN = scaler.inverse_transform(np.array(preds[-1]).reshape(-1, 1))
KEN = KEN.astype(float)
